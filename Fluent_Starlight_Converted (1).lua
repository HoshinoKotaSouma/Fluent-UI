...
return Library, SaveManager, InterfaceManager, Mobile\n\n-- ===== BEGIN STARLIGHT INTEGRATION (auto-merged) =====\n-- Appended by converter v2\n\n-- Starlight themes (extracted)\nlocal Themes = {
	...
	--Serika = {},
	--Rust = {}
}\n\ndo\n  if Creator and Creator.Themes and type(StarlightThemes) == 'table' then\n    for k,v in pairs(StarlightThemes) do Creator.Themes[k] = v end\n  end\nend\n\n-- Starlight topbar + profile helper (mac-like buttons + avatar + name)\ndo\n  local Players = game:GetService('Players')\n  local LocalPlayer = Players.LocalPlayer\n  if Library then\n    function Library:EnableStarlightTopbar(Window)\n      pcall(function()\n        if not Window or not Window.Root then return end\n        local top = Window.Root\n        local mac = Instance.new('Frame')\n        mac.Name='StarlightMacButtons' mac.Size = UDim2.new(0,88,0,24) mac.Position = UDim2.new(0,8,0,6) mac.BackgroundTransparency = 1 mac.Parent = top\n        local function mk(x,color,cb) local b=Instance.new('TextButton') b.Size=UDim2.fromOffset(12,12) b.Position=UDim2.fromOffset(x,6) b.BackgroundColor3=color b.BorderSizePixel=0 b.Text='' Instance.new('UICorner',b).CornerRadius=UDim.new(1,0) b.Parent=mac b.MouseButton1Click:Connect(cb) return b end\n        mk(0, Color3.fromRGB(255,95,86), function() pcall(function() if Library.Destroy then Library:Destroy() elseif Window.Destroy then Window:Destroy() end end) end)\n        mk(20, Color3.fromRGB(255,189,74), function() pcall(function() if Window.Minimize then Window:Minimize() end end) end)\n        mk(40, Color3.fromRGB(39,201,63), function() pcall(function() if Library._StarlightMaximized then if Library.Window and Library._StarlightPrev then Library.Window.Root.Size=Library._StarlightPrev.Size Library.Window.Root.Position=Library._StarlightPrev.Position end Library._StarlightMaximized=false else if Library.Window and Library.Window.Root then Library._StarlightPrev={Size=Library.Window.Root.Size,Position=Library.Window.Root.Position} Library.Window.Root.Size=UDim2.new(1,0,1,0) Library.Window.Root.Position=UDim2.new(0,0,0,0) end Library._StarlightMaximized=true end end) end)\n        local ph = Instance.new('Frame') ph.Name='StarlightProfile' ph.Size=UDim2.new(0,160,0,24) ph.Position=UDim2.new(1,-170,0,4) ph.BackgroundTransparency=1 ph.Parent=top\n        local thumb = Instance.new('ImageLabel') thumb.Size=UDim2.fromOffset(20,20) thumb.Position=UDim2.fromOffset(4,2) thumb.BackgroundTransparency=1 thumb.Parent=ph pcall(function() if LocalPlayer and LocalPlayer.UserId then thumb.Image='rbxthumb://type=AvatarHeadShot&id='..tostring(LocalPlayer.UserId)..'&w=48&h=48' end end)\n        local name = Instance.new('TextLabel') name.Parent=ph name.Position=UDim2.fromOffset(28,0) name.Size=UDim2.new(1,-28,1,0) name.BackgroundTransparency=1 name.Text=LocalPlayer and (LocalPlayer.DisplayName or LocalPlayer.Name) or 'Player' name.TextXAlignment=Enum.TextXAlignment.Left name.Font=Enum.Font.Gotham name.TextSize=14 name.TextColor3=Color3.new(1,1,1)\n      end)\n    end\n    pcall(function() if Library.Window then task.delay(0.6, function() Library:EnableStarlightTopbar(Library.Window) end) end end)\n  end\nend\n-- ===== END STARLIGHT INTEGRATION =====\n
-- Export thư viện
...
-- Thêm các phương thức chính
Fluent.CreateWindow = CreateWindow
Fluent.Version = Library.Version
Fluent.Themes = Library.Themes
Fluent.Windows = Library.Windows
Fluent.Options = Library.Options
Fluent.Theme = "Dark"  -- Theme mặc định

-- Thêm các phương thức khác từ Library
for k, v in pairs(Library) do
    if not Fluent[k] then
        Fluent[k] = v
    end
end

-- Thêm Creator vào Fluent
Fluent.Creator = Creator
